...
import { X, Drop, Bug, Scissors, ClockCounterClockwise, FloppyDisk, Warning, Plant } from "@phosphor-icons/react";
...
export function BitacoraModal({ batch, onClose }: { batch: any, onClose: () => void }) {
  ...
  const handleCreateEvent = async (e: React.FormEvent) => {
    ...
    if (actionType === 'riego' || actionType === 'ipm') {
       ...
       finalDesc = `MANEJO: ${manejoType}. ` + desc;
    ...
    const payload = {
      ...
      // amount_applied: amount ? parseFloat(amount) : null, // Removed to avoid schema cache error
      // product_id: selectedProduct || null, // Removed to avoid schema cache error
        ...
        <div className="flex-1 overflow-y-auto flex flex-col md:flex-row">
            ...
            <div className="p-6 md:w-1/2 border-r border-panel-border/30 bg-black/10">
               ...
               <div className="flex flex-wrap gap-2 mb-6">
                 <button onClick={() => setActionType('riego')} className={`flex-1 min-w-[65px] flex flex-col items-center gap-1 p-2 rounded-lg border transition-all ${actionType === 'riego' ? 'bg-blue-500/20 border-blue-500 text-blue-400' : 'bg-black/[0.03] dark:bg-black/20 border-panel-border text-brand-slate-600 hover:border-blue-500/50'}`}>
                    <Drop size={20} weight={actionType === 'riego' ? 'fill' : 'regular'} />
                    <span className="text-[10px] font-bold">RIEGO</span>
                 </button>
                 <button onClick={() => setActionType('ipm')} className={`flex-1 min-w-[65px] flex flex-col items-center gap-1 p-2 rounded-lg border transition-all ${actionType === 'ipm' ? 'bg-red-500/20 border-red-500 text-red-400' : 'bg-black/[0.03] dark:bg-black/20 border-panel-border text-brand-slate-600 hover:border-red-500/50'}`}>
                    <Bug size={20} weight={actionType === 'ipm' ? 'fill' : 'regular'} />
                    <span className="text-[10px] font-bold">IPM</span>
                 </button>
                 <button onClick={() => setActionType('poda')} className={`flex-1 min-w-[65px] flex flex-col items-center gap-1 p-2 rounded-lg border transition-all ${actionType === 'poda' ? 'bg-green-500/20 border-green-500 text-green-400' : 'bg-black/[0.03] dark:bg-black/20 border-panel-border text-brand-slate-600 hover:border-green-500/50'}`}>
                    <Scissors size={20} weight={actionType === 'poda' ? 'fill' : 'regular'} />
                    <span className="text-[10px] font-bold">PODA</span>
                 </button>
                 <button onClick={() => setActionType('manejo')} className={`flex-1 min-w-[65px] flex flex-col items-center gap-1 p-2 rounded-lg border transition-all ${actionType === 'manejo' ? 'bg-purple-500/20 border-purple-500 text-purple-400' : 'bg-black/[0.03] dark:bg-black/20 border-panel-border text-brand-slate-600 hover:border-purple-500/50'}`}>
                    <Plant size={20} weight={actionType === 'manejo' ? 'fill' : 'regular'} />
                    <span className="text-[10px] font-bold">MANEJO</span>
                 </button>
                 <button onClick={() => setActionType('alerta')} className={`flex-1 min-w-[65px] flex flex-col items-center gap-1 p-2 rounded-lg border transition-all ${actionType === 'alerta' ? 'bg-yellow-500/20 border-yellow-500 text-yellow-400' : 'bg-black/[0.03] dark:bg-black/20 border-panel-border text-brand-slate-600 hover:border-yellow-500/50'}`}>
               ...
               <form onSubmit={handleCreateEvent} className="flex flex-col gap-4">
                  ...
                  {actionType === 'manejo' && (
                    <div>
                       <label className="text-xs uppercase text-brand-slate-600 mb-1 block">Tarea / Manejo Estructural</label>
                       <select 
                          value={manejoType} 
                  ...
                  <button disabled={isSubmitting} type="submit" className={`w-full flex items-center justify-center gap-2 py-3 rounded-lg font-bold transition-all disabled:opacity-50 mt-2 ${actionType === 'riego' ? 'bg-blue-600 hover:bg-blue-500 text-foreground shadow-[0_0_15px_rgba(37,99,235,0.2)]' : actionType === 'ipm' ? 'bg-red-600 hover:bg-red-500 text-foreground shadow-[0_0_15px_rgba(220,38,38,0.2)]' : actionType === 'manejo' ? 'bg-purple-600 hover:bg-purple-500 text-foreground shadow-[0_0_15px_rgba(147,51,234,0.2)]' : actionType === 'alerta' ? 'bg-yellow-600 hover:bg-yellow-500 text-foreground shadow-[0_0_15px_rgba(234,179,8,0.2)]' : 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-[0_0_15px_rgba(16,185,129,0.2)]'}`}>
               ...
               <div className="flex-1 flex flex-col gap-3 relative pr-1 overflow-y-auto">
                  {loadingEvents ? (
                      ...
                      events.map(ev => {
                          ...
                          let EvIcon = Scissors;
                          if (ev.event_type === 'nutricion' || ev.event_type === 'riego' || ev.event_type === 'Riego') {
                              evColor = 'text-blue-400 border-blue-500/20 bg-blue-500/5'; EvIcon = Drop;
                          } else if (ev.event_type === 'ipm' || ev.event_type === 'IPM') {
                              evColor = 'text-red-400 border-red-500/20 bg-red-500/5'; EvIcon = Bug;
                          } else if (ev.event_type === 'alerta' || ev.event_type === 'Alerta') {
                              evColor = 'text-yellow-400 border-yellow-500/40 bg-yellow-500/10 shadow-[0_0_15px_rgba(234,179,8,0.1)]'; EvIcon = Warning;
                          } else if (ev.event_type === 'manejo' || ev.event_type === 'Manejo') {
                              evColor = 'text-purple-400 border-purple-500/20 bg-purple-500/5'; EvIcon = Plant;
                          }
