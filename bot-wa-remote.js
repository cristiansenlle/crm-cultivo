...
client.on('message_create', async (msg) => {
    ...
    const validIds = ['5491156548820@c.us', '228612670267594@lid'];
    const isSelfChat = (msg.from === msg.to && validIds.includes(msg.from)) || (validIds.includes(msg.from) && validIds.includes(msg.to));
    ...
    if (msg.from.includes('@g.us') || msg.to.includes('@g.us') || msg.from.includes('@broadcast')) {
        return;
    }
    ...
    try {
        if (msg.hasMedia) {
            ...
            if (media && media.mimetype && (media.mimetype.includes('audio') || media.mimetype.includes('ogg'))) {
                console.log('[WA] Audio detectado. Transcribiendo con Groq...');
                const FormData = require('form-data');
                const formData = new FormData();
                const buffer = Buffer.from(media.data, 'base64');
                const ext = media.mimetype.includes('mp4') ? 'm4a' : 'ogg';
                formData.append('file', buffer, { filename: 'audio.' + ext, contentType: media.mimetype });
                ...
                if (groqRes.data && groqRes.data.text) {
                    msg.body = groqRes.data.text;
                    console.log(`[WA] Transcripción exitosa: "${msg.body}"`);
                }
            }
        }
    } catch(e) {
        console.error('[WA] Error transcribiendo audio:', e.response ? e.response.data : e.message);
    }

    console.log(`[WA] Texto Final: "${msg.body}"`);
