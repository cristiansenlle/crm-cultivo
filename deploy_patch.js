...
async function runSQL() {
    try {
        ...
        const sql = fs.readFileSync('update_wf.sql', 'utf8');
        console.log('SQL Size:', sql.length, 'bytes');

        // Execute SQL securely via SSH stream
        const cmd = await ssh.execCommand('cat > /tmp/update_wf.sql', {
            options: {
                pty: false
            },
            stdin: sql
        ...
        const res = await ssh.execCommand('sqlite3 /root/.n8n/database.sqlite < /tmp/update_wf.sql');
