...
export function DashboardControls() {
  ...
  const handlePostTelemetry = async (e: React.FormEvent) => {
    ...
    // VALIDACIÓN CRUZADA: Verificar que el sensor pertenece a la sala activa
    if (activeSensor.room_id !== selectedRoom.id) {
      alert("⚠️ Error de contexto: El sensor seleccionado no pertenece a la sala activa. Recargando sensores...");
    ...
    try {
        ...
        if (error) {
            ...
            setTemperature("");
            setHumidity("");
            alert(`✅ Calibración inyectada a la red vía Sensor ${activeSensor.name} en sala ${selectedRoom.name}`);
        }
    } catch (error) {
