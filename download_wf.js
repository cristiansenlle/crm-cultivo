...
async function run() {
    ...
    console.log('Exporting workflow 1 from server...');
    const result = await ssh.execCommand('n8n export:workflow --id=1 --output=/tmp/wf_export.json');
    ...
    if (malformedCount === 0) {
        console.log('All tool nodes have a name parameter.');
        // Check node main names
