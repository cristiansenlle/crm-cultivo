        ...
        <div className="flex flex-col gap-6 w-full max-w-6xl mx-auto pb-10">
            ...
            <GlassCard className="w-full relative overflow-hidden p-8 flex flex-col md:flex-row justify-between md:items-center gap-6 border-l-4 border-amber-500">
                ...
                <div className="z-10">
                   <button 
                     ...
                     className="flex items-center gap-2 px-6 py-3 bg-amber-600 hover:bg-amber-500 text-foreground rounded-lg transition-transform hover:scale-105 shadow-[0_0_20px_rgba(245,158,11,0.3)] font-bold uppercase tracking-wider text-sm"
                   >
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {protocols.map((p, idx) => (
                        <div key={idx} onClick={() => openViewModal(p)} className="p-6 rounded-2xl bg-black/[0.03] dark:bg-black/20 border border-panel-border/50 hover:bg-black/[0.05] dark:bg-black/30 hover:border-amber-500/50 transition-all cursor-pointer group flex flex-col h-[250px]">
                            ...
                            <p className="text-brand-slate-600 dark:text-slate-400 text-sm font-mono overflow-hidden opacity-70 mb-4 flex-grow line-clamp-3">
                                {p.content}
                            </p>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
                    <GlassCard className="max-w-2xl w-full p-6 shadow-2xl relative">
                        <button onClick={() => setIsModalOpen(false)} className="absolute top-4 right-4 text-brand-slate-600 hover:text-foreground transition-colors p-1 bg-black/[0.03] dark:bg-black/20 rounded"><AppWindow size={24}/></button>
                        ...
                        <form onSubmit={handleSave} className="flex flex-col gap-4">
                            <div>
                                ...
                                <input type="text" required value={title} onChange={e=>setTitle(e.target.value)} className="w-full bg-black/[0.03] dark:bg-black/20 border border-panel-border rounded p-2 text-sm focus:border-amber-500 outline-none"/>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-xs font-mono text-brand-slate-600 uppercase mb-1 block">Fase Cultivo</label>
                                    <select value={stage} onChange={e=>setStage(e.target.value)} className="w-full bg-black/[0.03] dark:bg-black/20 border border-panel-border rounded p-2 text-sm focus:border-amber-500 outline-none">
                                ...
                                <div>
                                    ...
                                    <select value={topic} onChange={e=>setTopic(e.target.value)} className="w-full bg-black/[0.03] dark:bg-black/20 border border-panel-border rounded p-2 text-sm focus:border-amber-500 outline-none">
                            ...
                            <div>
                                ...
                                <textarea required value={content} onChange={e=>setContent(e.target.value)} rows={6} className="w-full bg-black/[0.03] dark:bg-black/20 border border-panel-border rounded p-2 text-sm font-mono focus:border-amber-500 outline-none resize-none"></textarea>
                            </div>
                            <button type="submit" className="mt-2 w-full bg-amber-600 hover:bg-amber-500 text-foreground font-bold py-3 rounded-lg shadow-lg">GUARDAR MANUAL</button>
                        </form>
                    </GlassCard>
                </div>
            )}
            ...
            {isViewModalOpen && activeProtocol && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                    <GlassCard className="max-w-3xl w-full p-8 shadow-2xl relative max-h-[90vh] overflow-y-auto">
                        <button onClick={() => setIsViewModalOpen(false)} className="absolute top-6 right-6 text-brand-slate-600 hover:text-foreground transition-colors p-1 bg-black/[0.03] dark:bg-black/20 rounded"><AppWindow size={24}/></button>
                        ...
                        <div className="font-mono text-[15px] leading-relaxed text-foreground whitespace-pre-wrap bg-black/[0.03] dark:bg-black/20 border border-panel-border/30 p-6 rounded-xl">
                        ...
                        <div className="mt-8 pt-4 border-t border-panel-border/50 flex justify-between items-center">
                            ...
                            <button onClick={() => openEditModal(activeProtocol)} className="bg-panel-border hover:bg-white/10 text-foreground px-6 py-2 rounded text-sm font-bold flex items-center gap-2 transition-colors"><PencilSimple size={18}/> Editar</button>
                        </div>
                    </GlassCard>
                </div>
            )}
