  ...
  return (
    <div className="fixed inset-0 z-[60] flex justify-end bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-panel-base border-l border-panel-border w-full max-w-2xl h-full shadow-2xl flex flex-col slide-in-from-right duration-300">
        ...
        {/* Header */}
        <div className="p-6 border-b border-panel-border bg-black/20 flex justify-between items-center">
           <div>
           ...
           <button onClick={onClose} className="text-brand-slate-600 hover:text-white transition-colors bg-black/40 p-2 rounded-lg">
             <X size={24} />
           </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto flex flex-col md:flex-row">
            ...
            <div className="p-6 md:w-1/2 border-r border-panel-border/30 bg-black/10">
               ...
               <div className="grid grid-cols-2 gap-2 mb-6">
                 <button onClick={() => setActionType('riego')} className={`flex flex-col items-center gap-1 p-2 rounded-lg border transition-all ${actionType === 'riego' ? 'bg-blue-500/20 border-blue-500 text-blue-400' : 'bg-black/20 border-panel-border text-brand-slate-600 hover:border-blue-500/50'}`}>
                    <Drop size={20} weight={actionType === 'riego' ? 'fill' : 'regular'} />
                    <span className="text-[10px] font-bold">RIEGO</span>
                 </button>
                 <button onClick={() => setActionType('ipm')} className={`flex flex-col items-center gap-1 p-2 rounded-lg border transition-all ${actionType === 'ipm' ? 'bg-red-500/20 border-red-500 text-red-400' : 'bg-black/20 border-panel-border text-brand-slate-600 hover:border-red-500/50'}`}>
                    <Bug size={20} weight={actionType === 'ipm' ? 'fill' : 'regular'} />
                    <span className="text-[10px] font-bold">IPM</span>
                 </button>
                 <button onClick={() => setActionType('poda')} className={`flex flex-col items-center gap-1 p-2 rounded-lg border transition-all ${actionType === 'poda' ? 'bg-green-500/20 border-green-500 text-green-400' : 'bg-black/20 border-panel-border text-brand-slate-600 hover:border-green-500/50'}`}>
                    <Scissors size={20} weight={actionType === 'poda' ? 'fill' : 'regular'} />
                    <span className="text-[10px] font-bold">PODA</span>
                 </button>
                 <button onClick={() => setActionType('alerta')} className={`flex flex-col items-center gap-1 p-2 rounded-lg border transition-all ${actionType === 'alerta' ? 'bg-yellow-500/20 border-yellow-500 text-yellow-400' : 'bg-black/20 border-panel-border text-brand-slate-600 hover:border-yellow-500/50'}`}>
               ...
               <form onSubmit={handleCreateEvent} className="flex flex-col gap-4">
                  {(actionType === 'riego' || actionType === 'ipm') && (
                    ...
                    <div>
                       ...
                       <select 
                          ...
                          className="w-full bg-black/20 border border-panel-border rounded-lg p-3 text-sm focus:border-blue-500 outline-none truncate"
                       >
                    ...
                    {selectedProduct && (
                        <div>
                           ...
                           <input type="number" step="0.1" required value={amount} onChange={(e)=>setAmount(e.target.value)} placeholder="Ej: 50" className="w-full bg-black/20 border border-emerald-500/30 rounded-lg p-3 text-sm focus:border-emerald-500 outline-none shadow-[0_0_15px_rgba(16,185,129,0.1)_inset]" />
                  ...
                  {actionType === 'poda' && (
                    <div>
                       ...
                       <select 
                          ...
                          className="w-full bg-black/20 border border-panel-border rounded-lg p-3 text-sm focus:border-green-500 outline-none"
                       >
                  ...
                  <div>
                    ...
                    <textarea required={actionType === 'alerta'} value={desc} onChange={e=>setDesc(e.target.value)} rows={2} placeholder="Comentarios adicionales..." className="w-full bg-black/20 border border-panel-border rounded-lg p-3 text-sm focus:border-emerald-500 outline-none resize-none"></textarea>
                  </div>

                  <button disabled={isSubmitting} type="submit" className={`w-full flex items-center justify-center gap-2 py-3 rounded-lg font-bold transition-all disabled:opacity-50 mt-2 ${actionType === 'riego' ? 'bg-blue-600 hover:bg-blue-500 text-white shadow-[0_0_15px_rgba(37,99,235,0.2)]' : actionType === 'ipm' ? 'bg-red-600 hover:bg-red-500 text-white shadow-[0_0_15px_rgba(220,38,38,0.2)]' : actionType === 'alerta' ? 'bg-yellow-600 hover:bg-yellow-500 text-white shadow-[0_0_15px_rgba(234,179,8,0.2)]' : 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-[0_0_15px_rgba(16,185,129,0.2)]'}`}>
            ...
            <div className="p-6 md:w-1/2 flex flex-col">
               <h3 className="text-sm uppercase tracking-widest font-bold text-brand-slate-600 mb-6 flex justify-between items-center relative pl-3 border-l-2 border-brand-slate-600">
                  ...
                  <span className="text-[10px] bg-black/40 px-2 py-0.5 rounded text-emerald-500">{events.length} logs vitales</span>
               </h3>
                          ...
                          return (
                              <div key={ev.id} className={`p-4 rounded-xl border ${evColor} flex flex-col gap-2 shadow-sm`}>
                                  <div className="flex justify-between items-start">
                                     ...
                                     <span className="text-[10px] font-mono opacity-80 bg-black/40 px-2 py-1 rounded">{new Date(ev.date_occurred || ev.timestamp || ev.created_at).toLocaleDateString()}</span>
                                  </div>
