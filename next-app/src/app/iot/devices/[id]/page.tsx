        ...
        <div className="space-y-6 pt-4">
          ...
          <div className="bg-panel-base backdrop-blur-md border border-panel-border rounded-xl p-6">
            ...
            <div className="flex items-end gap-4 max-w-2xl">
              ...
              <button onClick={createSchedule} disabled={loading} className="bg-emerald-600 hover:bg-emerald-500 text-foreground font-medium py-2 px-6 rounded-lg">{loading ? '...' : '+ Agregar'}</button>
            </div>
          </div>
          <div className="bg-panel-base backdrop-blur-md border border-panel-border rounded-xl overflow-hidden">
      ...
      {activeTab === 'rules' && (
        <div className="space-y-6 pt-4">
          <div className="bg-panel-base backdrop-blur-md border border-panel-border rounded-xl p-6">
            ...
            <p className="text-sm text-foreground/60 dark:text-foreground/60 mb-6">El script vivirá en este dispositivo y enviará comandos MQTT al otro Shelly con precisión milimétrica.</p>
            <div className="grid grid-cols-5 items-end gap-4">
              <div className="col-span-1"><label className="block text-sm font-medium text-foreground/60 dark:text-foreground/60 mb-1">Si este se...</label><select value={ruleTrigger} onChange={e => setRuleTrigger(e.target.value)} className="w-full bg-background border border-panel-border rounded-lg px-4 py-2 text-foreground"><option value="on">Enciende</option><option value="off">Apaga</option></select></div>
              <div className="col-span-1"><label className="block text-sm font-medium text-foreground/60 dark:text-foreground/60 mb-1">Retraso (Segs)</label><input type="number" min="0" value={ruleDelay} onChange={e => setRuleDelay(Number(e.target.value))} className="w-full bg-background border border-panel-border rounded-lg px-4 py-2 text-foreground" /></div>
              <div className="col-span-2"><label className="block text-sm font-medium text-foreground/60 dark:text-foreground/60 mb-1">Entonces el Dispositivo...</label><select value={ruleTargetDev} onChange={e => setRuleTargetDev(e.target.value)} className="w-full bg-background border border-panel-border rounded-lg px-4 py-2 text-foreground">{Object.values(MOCK_DEVICES).map(d => <option key={d.id} value={d.deviceId}>{d.name} ({d.deviceId})</option>)}</select></div>
              <div className="col-span-1"><label className="block text-sm font-medium text-foreground/60 dark:text-foreground/60 mb-1">Debe...</label><select value={ruleAction} onChange={e => setRuleAction(e.target.value)} className="w-full bg-background border border-panel-border rounded-lg px-4 py-2 text-foreground"><option value="on">Encenderse</option><option value="off">Apagarse</option></select></div>
            </div>
