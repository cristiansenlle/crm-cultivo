...
export default function DevicePage() {
  ...
  const [activeTab, setActiveTab] = useState<'control' | 'schedules' | 'rules'>('control');
  ...
  useEffect(() => {
    if (device) {
      ...
      fetch('/api/iot/shelly', {
        ...
        if (data.success && data.result) setIsOn(data.result['switch:0']?.output || false);
      });
  ...
  return (
    <div className="p-8 space-y-6 bg-zinc-950 min-h-screen text-white">
      ...
      <div className="flex space-x-2 border-b border-zinc-800 pb-px">
        {['control', 'schedules', 'rules'].map(tab => (
          ...
          >
            {tab === 'control' ? 'Control Manual' : tab === 'schedules' ? 'Programación (Timers)' : 'Automatizaciones'}
          </button>
        ))}
      </div>
