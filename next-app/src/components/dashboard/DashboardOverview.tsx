...
function SensorDashboard({ sensor }: { sensor: any }) {
  ...
  return (
    <div className="flex flex-col gap-4 border border-panel-border/30 bg-black/5 dark:bg-white/5 rounded-2xl p-4 shadow-xl">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <TelemetryRadiant type="vpd" value={latestMetric?.vpd || 0} status={latestMetric ? "optimal" : "danger"} />
      </section>
...
export function DashboardOverview() {
  ...
  return (
    <div className="flex flex-col gap-8 w-full">
       {sensors.map(s => <SensorDashboard key={s.id} sensor={s} />)}
    </div>
  );
}
