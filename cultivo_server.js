...
let batches = [];
let rooms = []; // Loaded from core_rooms
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Cargar Salas de Cultivo en el Selector de Ubicación
...
async function loadRoomsDropdown() {
    const sel = document.getElementById('cropLocation');
    if (!sel) return;

    if (!window.sbClient) return;

    try {
        const { data, error } = await window.sbClient.from('core_rooms').select('*').order('created_at', { ascending: true });
        ...
        rooms.forEach(room => {
            ...
            opt.value = room.id; // Use UUID as value
            opt.text = room.name + (room.phase ? ` (${room.phase})` : '');
            sel.appendChild(opt);
        });

        if (rooms.length === 0) {
            sel.innerHTML = '<option value="">No hay salas. Crea una en el Panel Principal.</option>';
        }
    } catch (e) {
...
async function loadBatches() {
    ...
    if (!window.sbClient) {
        if (list) list.innerHTML = `<li style="padding: 30px; text-align: center; color: var(--text-secondary); list-style: none;"><i class="ph ph-spinner" style="font-size: 1.5rem;"></i><br>Conectando...</li>`;
        // Retry after 500ms in case sbClient is still loading
        setTimeout(loadBatches, 500);
        return;
    }
    if (list) list.innerHTML = `<li style="padding: 30px; text-align: center; color: var(--text-secondary); list-style: none;"><i class="ph ph-circle-notch" style="font-size: 1.5rem;"></i><br>Cargando lotes...</li>`;
    try {
        ...
        if (list) list.innerHTML = `<li style="padding: 30px; text-align: center; color: var(--color-red); list-style: none;">Error cargando lotes: ${e.message}</li>`;
    }
}
...
async function handleNewCrop(e) {
    ...
    if (syncGCal) {
        try {
            const webhookCalendarUrl = "http://109.199.99.126:5678/webhook/cultivo-calendar";
...
function renderBatches() {
    ...
    batches.forEach((b, index) => {
        ...
        // Mapear location con el nombre de la sala desde el array rooms
        let roomName = b.location || 'Sin Sala';
        const roomMatch = rooms.find(r => r.id === b.location || r.name === b.location);
        if (roomMatch) roomName = roomMatch.name + (roomMatch.phase ? ` (${roomMatch.phase})` : '');
        ...
        if (b.stage === 'cosecha') {
            if (b.weightDry) {
                actionHtml = `<div style="display:flex; gap:8px;">
                    <span style="font-size:0.85rem; color:var(--color-yellow); font-weight:bold;"><i class="ph ph-check"></i> En Stock (${b.weightDry}g)</span>
                    <button class="btn-primary" style="padding:7px 12px; border-color:var(--color-red); color:var(--color-red); background:transparent;" onclick="deleteBatch('${b.id}')" title="Eliminar Lote"><i class="ph ph-trash"></i></button>
                </div>`;
            } else {
                actionHtml = `<div style="display:flex; gap:8px;">
                    <button class="btn-primary" style="padding:8px 15px; border-color:var(--color-yellow); color:var(--color-yellow); background:transparent;" onclick="openDryWeightModal('${b.id}')">Cargar Seco</button>
                    <button class="btn-primary" style="padding:7px 12px; border-color:var(--color-red); color:var(--color-red); background:transparent;" onclick="deleteBatch('${b.id}')" title="Eliminar Lote"><i class="ph ph-trash"></i></button>
                </div>`;
            }
        } else {
            // Active batch (not cosecha) - show full action bar
            actionHtml = `
                <div style="display:flex; gap:8px; flex-wrap:wrap; justify-content:flex-end;">
                    <button class="btn-primary" style="padding:7px 12px; border-color:var(--text-secondary); color:var(--text-secondary); background:transparent;" onclick="viewAgronomicHistory('${b.id}')" title="Historial"><i class="ph ph-clock-counter-clockwise"></i></button>
                    <button class="btn-primary" style="padding:7px 12px; border-color:var(--color-blue); color:var(--color-blue); background:transparent;" onclick="openAgronomicEventModal('${b.id}', '${b.location}')" title="Evento"><i class="ph ph-bug"></i></button>
                    <button class="btn-icon" onclick="advanceStage('${b.id}')" title="Avanzar Fase"><i class="ph ph-flag-checkered"></i></button>
                    <button class="btn-primary" style="padding:7px 12px; border-color:var(--color-yellow); color:var(--color-yellow); background:transparent;" onclick="openEditBatchModal('${b.id}')" title="Editar Lote"><i class="ph ph-pencil-simple"></i></button>
                    <button class="btn-primary" style="padding:7px 12px; border-color:var(--color-red); color:var(--color-red); background:transparent;" onclick="deleteBatch('${b.id}')" title="Eliminar Lote"><i class="ph ph-trash"></i></button>
                </div>
            `;
        }
        ...
        <li class="task-item" style="border-left: 3px solid ${color}; flex-direction: column; align-items: stretch; padding: 15px; background: var(--bg-panel);">
            ...
            <details style="background: var(--bg-dark); padding: 10px; border-radius: 6px; margin-bottom: 8px; border: 1px solid #333;">
                ...
                <div style="margin-top: 10px; padding-top: 10px; border-top: 1px solid #444; display: flex; gap: 10px; align-items: flex-end; flex-wrap: wrap;">
                    ...
                    <div style="flex: 1 1 80px;"><label style="font-size:0.75rem;">Vol/Pl</label><input type="text" id="nutri-vol-${index}" placeholder="Ej: 1L" style="width:100%; padding:5px; border-radius:4px; background:#111; color:#fff; border:1px solid #444;"></div>
    ...
    try {
        ...
        if (!data || data.length === 0) {
            ...
            sortedData.forEach(ev => {
                let detailStr = '';
                if (ev.event_type === 'nutricion') {
                    detailStr = `Prod: ${ev.details.product_name} | Cant: ${ev.amount_applied || 0} | EC/pH: ${ev.details.ec_ph} | Vol: ${ev.details.vol}`;
                } else if (ev.event_type === 'ipm') {
                    detailStr = `Acción: ${ev.details.product_name} | Cant: ${ev.amount_applied || 0}`;
                } else if (ev.event_type === 'poda') {
                    detailStr = `Tipo: ${ev.details.prune_type}`;
                } else if (ev.description) {
                    detailStr = ev.description;
                } else {
                    detailStr = ev.details ? JSON.stringify(ev.details) : 'Sin detalles';
                }

                const eventDate = ev.date_occurred || ev.timestamp || ev.created_at;
                const tr = document.createElement('tr');
                tr.innerHTML = `
                    <td>${eventDate ? new Date(eventDate).toLocaleDateString() : 'N/A'} ${eventDate ? new Date(eventDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}</td>
...
function ensureEditModal() {
    if (document.getElementById('modal-edit-batch')) return;
    const modal = document.createElement('div');
    modal.id = 'modal-edit-batch';
    ...
    modal.innerHTML = `
        <div style="background:var(--bg-panel); border:1px solid #333; border-radius:12px; padding:30px; max-width:520px; width:90%; max-height:90vh; overflow-y:auto;">
            <h3 style="margin:0 0 20px; display:flex; align-items:center; gap:10px;"><i class="ph ph-pencil-simple"></i> Editar Lote</h3>
            <input type="hidden" id="edit-batch-original-id">
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:12px;">
                <div>
                    <label style="font-size:0.75rem; color:var(--text-secondary);">ID del Lote</label>
                    <input id="edit-batch-id" type="text" style="width:100%; padding:8px; border-radius:6px; background:#111; color:#fff; border:1px solid #444; box-sizing:border-box;">
                </div>
                <div>
                    <label style="font-size:0.75rem; color:var(--text-secondary);">Cepa / Genética</label>
                    <input id="edit-batch-strain" type="text" style="width:100%; padding:8px; border-radius:6px; background:#111; color:#fff; border:1px solid #444; box-sizing:border-box;">
                </div>
                <div>
                    <label style="font-size:0.75rem; color:var(--text-secondary);">Fecha Inicio</label>
                    <input id="edit-batch-date" type="date" style="width:100%; padding:8px; border-radius:6px; background:#111; color:#fff; border:1px solid #444; box-sizing:border-box;">
                </div>
                <div>
                    <label style="font-size:0.75rem; color:var(--text-secondary);">Fase</label>
                    <select id="edit-batch-stage" style="width:100%; padding:8px; border-radius:6px; background:#111; color:#fff; border:1px solid #444; box-sizing:border-box;">
                ...
                <div>
                    <label style="font-size:0.75rem; color:var(--text-secondary);">Origen</label>
                    <select id="edit-batch-origen" style="width:100%; padding:8px; border-radius:6px; background:#111; color:#fff; border:1px solid #444; box-sizing:border-box;">
                        <option value="clon">Clon</option>
                        <option value="semilla">Semilla</option>
                    </select>
                </div>
                <div>
                    <label style="font-size:0.75rem; color:var(--text-secondary);">Madre (opcional)</label>
                    <input id="edit-batch-madre" type="text" style="width:100%; padding:8px; border-radius:6px; background:#111; color:#fff; border:1px solid #444; box-sizing:border-box;">
                </div>
                <div>
                    <label style="font-size:0.75rem; color:var(--text-secondary);">Sala de Ubicación</label>
                    <select id="edit-batch-location" style="width:100%; padding:8px; border-radius:6px; background:#111; color:#fff; border:1px solid #444; box-sizing:border-box;">
                        <option value="">-- Seleccionar --</option>
                    </select>
                </div>
                <div>
                    <label style="font-size:0.75rem; color:var(--text-secondary);">Días de Floración</label>
                    <input id="edit-batch-flowerdays" type="number" min="1" max="120" style="width:100%; padding:8px; border-radius:6px; background:#111; color:#fff; border:1px solid #444; box-sizing:border-box;">
                </div>
            </div>
            <div style="display:flex; gap:10px; justify-content:flex-end; margin-top:20px;">
                <button class="btn-primary" style="border-color:#555; color:#aaa;" onclick="closeEditBatchModal()">Cancelar</button>
                <button class="btn-primary" style="background:var(--color-yellow); border-color:var(--color-yellow); color:#000;" onclick="saveEditBatch()"><i class="ph ph-floppy-disk"></i> Guardar Cambios</button>
            </div>
        </div>`;
...
function openEditBatchModal(batchId) {
    ...
    document.getElementById('edit-batch-strain').value = batch.strain || '';
    document.getElementById('edit-batch-date').value = batch.startDate || '';
    document.getElementById('edit-batch-stage').value = batch.stage || 'germinacion';
    document.getElementById('edit-batch-origen').value = batch.origen || 'clon';
    document.getElementById('edit-batch-madre').value = batch.madre || '';
    document.getElementById('edit-batch-flowerdays').value = batch.flowerDays || '';

    // Populate rooms in the location dropdown
    const locSel = document.getElementById('edit-batch-location');
    locSel.innerHTML = '<option value="">-- Seleccionar Sala --</option>';
    rooms.forEach(room => {
        const opt = document.createElement('option');
        opt.value = room.id;
        opt.text = room.name + (room.phase ? ` (${room.phase})` : '');
        if (room.id === batch.location || room.name === batch.location) opt.selected = true;
        locSel.appendChild(opt);
    });
    // If no match, also add a text option for legacy string locations
    if (batch.location && !rooms.find(r => r.id === batch.location)) {
        const legacyOpt = document.createElement('option');
        legacyOpt.value = batch.location;
        legacyOpt.text = batch.location + ' (heredado)';
        legacyOpt.selected = true;
        locSel.appendChild(legacyOpt);
    }
...
function closeEditBatchModal() {
    const modal = document.getElementById('modal-edit-batch');
    if (modal) modal.style.display = 'none';
}

async function saveEditBatch() {
    const originalId = document.getElementById('edit-batch-original-id').value;
    const newId = document.getElementById('edit-batch-id').value.trim();
    const strain = document.getElementById('edit-batch-strain').value.trim();
    const startDate = document.getElementById('edit-batch-date').value;
    const stage = document.getElementById('edit-batch-stage').value;
    const origen = document.getElementById('edit-batch-origen').value;
    const madre = document.getElementById('edit-batch-madre').value.trim();
    const location = document.getElementById('edit-batch-location').value;
    const flowerDays = document.getElementById('edit-batch-flowerdays').value;

    if (!newId || !strain) return alert('El ID y la cepa son obligatorios.');

    if (!window.sbClient) return alert('Conexión a base de datos no disponible.');
    ...
    try {
        ...
        // If ID changed, we need to handle differently (text PK)
        if (newId !== originalId) {
            // Insert with new ID, copy data, delete old
            ...
            await window.sbClient.from('core_batches').delete().eq('id', originalId);
        } else {
            const { error } = await window.sbClient.from('core_batches').update(updateData).eq('id', originalId);
            if (error) throw error;
        }

        closeEditBatchModal();
        await loadBatches();
        console.log('✅ Lote actualizado:', newId);
    } catch (e) {
...
async function deleteBatch(batchId) {
    const confirmed = confirm(`¿Eliminar el lote "${batchId}"?\n\nEsta acción no se puede deshacer. Se eliminarán también los eventos agronómicos y datos de telemetría asociados.`);
    if (!confirmed) return;

    if (!window.sbClient) return alert('Conexión a base de datos no disponible.');

    try {
        // Delete related agronomic events first (to avoid FK issues)
        await window.sbClient.from('core_agronomic_events').delete().eq('batch_id', batchId);
        // Delete telemetry
        await window.sbClient.from('daily_telemetry').delete().eq('batch_id', batchId);
        // Delete the batch itself
        const { error } = await window.sbClient.from('core_batches').delete().eq('id', batchId);
        if (error) throw error;

        await loadBatches();
        console.log('🗑️ Lote eliminado:', batchId);
    } catch (e) {
