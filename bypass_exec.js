...
async function execCmdBypass() {
    try {
        ...
        const execNode = {
            "parameters": {
                "command": `node -e "const fetch = require('node-fetch'); const q = { batch_id: '{{$json.body.batch_id}}', room_id: '{{$json.body.batch_id}}', temperature_c: {{$json.body.temp}}, humidity_percent: {{$json.body.humidity}}, vpd_kpa: {{$json.body.vpd}}, created_at: '{{$json.body.timestamp}}' }; fetch('https://opnjrzixsrizdnphbjnq.supabase.co/rest/v1/daily_telemetry', { method: 'POST', headers: { 'apikey': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9wbmpyeml4c3JpemRucGhiam5xIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI1Nzc2NTAsImV4cCI6MjA4ODE1MzY1MH0.Hx6t3ZFZj6TGei8c99bfdwUfJ1ce5XaGM4IuUBJUeC8', 'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9wbmpyeml4c3JpemRucGhiam5xIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI1Nzc2NTAsImV4cCI6MjA4ODE1MzY1MH0.Hx6t3ZFZj6TGei8c99bfdwUfJ1ce5XaGM4IuUBJUeC8', 'Content-Type': 'application/json' }, body: JSON.stringify(q) }).then(r => console.log(r.status));"`
            },
