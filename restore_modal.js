...
function openAdjustModal(id, name, currentQty, currentUnitCost, unit) {
    document.getElementById('adjustModalData').innerHTML = \`
        ...
        <div style="background:var(--bg-light); padding:15px; border-radius:8px; border:1px solid var(--border-color); margin-bottom:15px;">
...
if (!js.includes("function openAdjustModal")) {
    js = js.replace("async function removeQuimico", missingFunctions + "\nasync function removeQuimico");
    fs.writeFileSync('c:/Users/Cristian/.gemini/antigravity/crm cannabis/insumos.js', js);
    console.log("Functions Restored");
} else {
    console.log("Functions were already there!");
}
