  ...
  return (
    <div className="flex flex-col gap-4 border border-blue-500/30 bg-black/5 dark:bg-white/5 rounded-2xl p-4 shadow-xl">
      ...
      <section className="grid grid-cols-1 md:grid-cols-1 gap-4">
        ...
        <div className="flex flex-col items-center justify-center p-6 bg-black/10 dark:bg-black/20 rounded-xl border border-blue-500/20">
      ...
      <section className="w-full h-64 mt-4">
...
export function DashboardOverview() {
  ...
  return (
    <div className="flex flex-col gap-8 w-full">
       {sensors.map(s => s.type === 'soil' ? (
           <SoilSensorDashboard key={s.id} sensor={s} />
       ) : (
           <SensorDashboard key={s.id} sensor={s} roomPhase={selectedRoom.phase} />
       ))}
    </div>
  );
}
