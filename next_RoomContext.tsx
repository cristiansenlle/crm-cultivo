...
export function RoomProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchSensors = async () => {
    ...
    if (data && !error) {
      setSensors(data);
      // CRITICAL: Always verify activeSensor belongs to the NEW room's sensor list.
      // If not found (e.g. after room switch), reset to first available sensor.
      const currentStillValid = activeSensor && data.some(s => s.id === activeSensor.id);
      if (data.length > 0 && !currentStillValid) {
