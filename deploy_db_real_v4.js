...
async function run() {
    ...
    await ssh.execCommand(`echo '${wfJson.replace(/'/g, "'\\''")}' > /root/temp_db_deploy2.json`);

    let importRes = await ssh.execCommand('n8n import:workflow --input=/root/temp_db_deploy2.json');
    console.log("Import STDOUT:", importRes.stdout);
    
    let dbRes = await ssh.execCommand('sqlite3 /root/.n8n/database.sqlite "SELECT id FROM workflow_entity WHERE name = \\\'DB Deploy 2\\\' ORDER BY \\"createdAt\\" DESC LIMIT 1;"');
